...
export class MysqlDialect extends SqlDialect {
  ...
  showTables(database: string): string {
    return `SELECT table_comment \`comment\`,TABLE_NAME as \`name\`,TABLE_ROWS \`rows\`,auto_increment,\`row_format\`,data_length,index_length FROM information_schema.TABLES  WHERE TABLE_SCHEMA = '${database}' order by table_name;`;
  }
  showSchemas(): string {
